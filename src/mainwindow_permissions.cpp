...
bool MainWindow::ensureDatasetPermissionsLoaded(int connIdx, const QString& poolName, const QString& datasetName) {
    ...
    if (!isLocalConnection(connIdx) && m_daemonTransport.isDaemonAvailable()) {
        ...
        const zfsmgr::DaemonRpcResult res =
            m_daemonTransport.call(QStringLiteral("/permissions"),
                                   QJsonObject{{QStringLiteral("dataset"), datasetName}},
                                   profile);
        if (res.code == 200) {
            const QJsonObject payload = res.payload;
            DatasetPermissionsCacheEntry entry;
            entry.loaded = true;
            auto readGrantArray = [](const QJsonArray& arr) {
                QVector<DatasetPermissionGrant> out;
                for (const QJsonValue& value : arr) {
                    const QJsonObject obj = value.toObject();
                    DatasetPermissionGrant grant;
                    grant.scope = obj.value(QStringLiteral("scope")).toString();
                    grant.targetType = obj.value(QStringLiteral("targetType")).toString();
                    grant.targetName = obj.value(QStringLiteral("targetName")).toString();
                    for (const QJsonValue& perm : obj.value(QStringLiteral("permissions")).toArray()) {
                        const QString token = perm.toString().trimmed();
                        if (!token.isEmpty()) {
                            grant.permissions.push_back(token);
                        }
                    }
                    out.push_back(grant);
                }
            ...
            entry.localGrants = readGrantArray(payload.value(QStringLiteral("localGrants")).toArray());
            entry.originalLocalGrants = entry.localGrants;
            entry.descendantGrants = readGrantArray(payload.value(QStringLiteral("descendantGrants")).toArray());
            entry.originalDescendantGrants = entry.descendantGrants;
            entry.localDescendantGrants =
                readGrantArray(payload.value(QStringLiteral("localDescendantGrants")).toArray());
            entry.originalLocalDescendantGrants = entry.localDescendantGrants;
            for (const QJsonValue& perm : payload.value(QStringLiteral("createPermissions")).toArray()) {
                const QString token = perm.toString().trimmed();
                if (!token.isEmpty()) {
                    entry.createPermissions.push_back(token);
                }
            }
            entry.originalCreatePermissions = entry.createPermissions;
            for (const QJsonValue& setValue : payload.value(QStringLiteral("permissionSets")).toArray()) {
                const QJsonObject obj = setValue.toObject();
                DatasetPermissionSet set;
                set.name = obj.value(QStringLiteral("name")).toString();
                for (const QJsonValue& perm : obj.value(QStringLiteral("permissions")).toArray()) {
                    const QString token = perm.toString().trimmed();
                    if (!token.isEmpty()) {
                        set.permissions.push_back(token);
                    }
                }
                if (!set.name.trimmed().isEmpty()) {
                    entry.permissionSets.push_back(set);
                    entry.originalPermissionSets.push_back(set);
                }
            }
            for (const QJsonValue& user : payload.value(QStringLiteral("systemUsers")).toArray()) {
                const QString token = user.toString().trimmed();
                if (!token.isEmpty()) {
                    entry.systemUsers.push_back(token);
                }
            }
            for (const QJsonValue& group : payload.value(QStringLiteral("systemGroups")).toArray()) {
                const QString token = group.toString().trimmed();
                if (!token.isEmpty()) {
                    entry.systemGroups.push_back(token);
                }
            }
        ...
        appLog(QStringLiteral("WARN"),
               QStringLiteral("Daemon permissions fallback to SSH %1::%2 (%3)")
                   .arg(m_profiles[connIdx].name, datasetName, oneLine(res.message.isEmpty()

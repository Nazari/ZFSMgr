...
void MainWindow::syncConnContentPropertyColumns() {
    ...
    auto querySnapshotHolds = [this](int connIdx, const QString& poolName, const QString& objectName) {
        ...
        if (connIdx < 0 || connIdx >= m_profiles.size() || poolName.trimmed().isEmpty() || objectName.trimmed().isEmpty()) {
            return holds;
        }
        ...
        const QString cmd = withSudo(
            p,
            QStringLiteral("zfs holds -H %1").arg(mwhelpers::shSingleQuote(objectName)));
    ...
    if (m_showInlinePropertyNodes && !propsNode) {
    ...
    if (datasetSupportsGsa && m_showInlineGsaNode) {
        if (!gsaNode) {
...
void MainWindow::populateDatasetTree(QTreeWidget* tree, int connIdx, const QString& poolName, const QString& side, bool allowRemoteLoadIfMissing) {
    ...
    for (const DatasetRecord& rec : cache.datasets) {
        ...
        if (isFilesystemRecord(rec) && m_showInlineGsaNode) {

...
QString MainWindow::connectionStateTooltipHtml(int connIdx) const {
    ...
    // ninguna parte: el usuario lo descubría al pulsar y ver fallar la acción.
    {
        ...
        const QVector<QPair<F, QString>> checked = {
            ...
            {F::SendRecvStreaming, QStringLiteral("Enviar/Nivelar entre máquinas")},
...
QString connectivityMatrixRemoteProbe(const ConnectionProfile& target, bool verbose = false) {
    ...
    if (!target.password.trimmed().isEmpty()) {
        return unixPathPrefix + QStringLiteral(
                   // La contraseña NO va aquí dentro. Entra por la ENTRADA ESTÁNDAR del
                   // ssh exterior, y `sshpass -d0` la lee de ahí.
                   //
                   // Antes esto era `SSHPASS=<contraseña> sshpass -e`, y esa cadena acaba
                   // siendo un argumento del ssh exterior: se veía en el `ps` de ESTE
                   // equipo, no solo en el remoto. Y `-e` tampoco valdría aunque se
                   // arreglara el argumento, porque el entorno de un proceso se lee en
                   // /proc/<pid>/environ. Ver docs/diseno_tecnico_conectividad.md.
                   "if command -v sshpass >/dev/null 2>&1; then "
                   "sshpass -d0 %1 -o BatchMode=no "
                   "-o PreferredAuthentications=password,keyboard-interactive,publickey "
                   "-o NumberOfPasswordPrompts=1 %2 %3; "
                   "else echo %4 >&2; exit 127; fi")
            .arg(sshBase,
                 targetHost,
                 targetCmd,
...
QString connectivityMatrixRsyncProbe(const ConnectionProfile& target) {
    ...
    const QString baseProbe = target.password.trimmed().isEmpty()
        ...
        : unixPathPrefix + QStringLiteral(
              // Igual que en la prueba de SSH: la contraseña por la entrada estándar,
              // nunca dentro de la orden.
              "if command -v sshpass >/dev/null 2>&1; then "
              "sshpass -d0 %1 -o BatchMode=no "
              "-o PreferredAuthentications=password,keyboard-interactive,publickey "
              "-o NumberOfPasswordPrompts=1 %2 %3; "
              "else echo %4 >&2; exit 127; fi")
              .arg(mwhelpers::sshBaseCommand(target),
                   targetHost,
                   targetCmd,
                   mwhelpers::shSingleQuote(QStringLiteral("ZFSMgr: sshpass no disponible para esta prueba")));
...
void MainWindow::openConnectivityMatrixDialog() {
    ...
    auto probeConnectivity = [&](int rowIdx, int colIdx) -> ConnectivityProbeResult {
        ...
        QString sshMerged;
        QString sshDetail;
        // La contraseña del destino viaja por la ENTRADA ESTÁNDAR, que es de donde la lee
        // `sshpass -d0` en el otro extremo. Dentro de la orden se vería con un `ps` aquí.
        const QByteArray claveDestino =
            effectiveDst.password.trimmed().isEmpty()
                ? QByteArray()
                : (effectiveDst.password.toUtf8() + '\n');
...
void MainWindow::refreshConnectionByIndex(int idx) {
    ...
    // while ensureDatasetsLoaded holds a reference into m_conns.poolDatasetCache.
    ...
    // Al refrescar una conexión, invalidar toda la caché asociada a todos sus pools.
    {
        ...
        while (dsIt != m_conns.poolDatasetCache.end()) {
            if (dsIt.key().startsWith(connPrefix)) {
                dsIt = m_conns.poolDatasetCache.erase(dsIt);
            } else {
                ++dsIt;
            }
        }
...
//
// Se pregunta antes, y se dice QUÉ se entrega: no son nombres de máquinas, son las claves
// privadas con las que esa máquina podrá hablar con todas como si fuera usted.
void MainWindow::pushPeerCredentialsToConnection(int connIdx) {
    if (connIdx < 0 || connIdx >= m_conns.profiles.size()) {
        return;
    }
    const ConnectionProfile destino = m_conns.profiles.at(connIdx);
    const QString nombreDestino = destino.id.trimmed().isEmpty() ? destino.name.trimmed()
                                                                 : destino.id.trimmed();

    std::vector<zfsmgr::base::ConnectionProfile> perfiles;
    perfiles.reserve(static_cast<std::size_t>(m_conns.profiles.size()));
    for (const ConnectionProfile& p : m_conns.profiles) {
    ...
    const auto choice = QMessageBox::question(
        this, QStringLiteral("ZFSMgr"),
        trk(QStringLiteral("t_push_peers_confirm001"),
            QStringLiteral("Se le van a entregar a «%1» las credenciales de: %2.\n\n"
                           "No es una lista de nombres: son las claves privadas con las que "
                           "esa máquina podrá hablar con las demás como si fuera usted. "
    ...
    if (choice != QMessageBox::Yes) {
        appLog(QStringLiteral("INFO"),
               QStringLiteral("[peers] entrega cancelada para %1").arg(nombreDestino));
        return;
    }
    ...
    appLog(QStringLiteral("NORMAL"),
           QStringLiteral("[peers] entregadas %1 credenciales a %2")
               .arg(nombres.size())
               .arg(nombreDestino));
    QMessageBox::information(this, QStringLiteral("ZFSMgr"),

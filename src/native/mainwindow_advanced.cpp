...
void MainWindow::actionAdvancedBreakdown(const DatasetSelectionContext& explicitCtx) {
    ...
    if (isWindowsConnection(p)) {
        // **Por el mismo verbo que en Unix.** Aquí había 40 líneas de PowerShell que
        // resolvían la letra de unidad subiendo por los padres y luego recorrían el árbol
        // con Get-ChildItem. `--dump-advanced-breakdown-list` hace exactamente eso, y ya
        // funcionaba en Windows: resuelve el punto de montaje con los montajes REALES
        // —`Z:/sa`, no la propiedad `/winpool/sa`— y recorre con std::filesystem.
        // Comprobado contra OldLau: contesta «__MP__=Z:/sa» y la lista de directorios.
        //
        // Es el mismo formato de salida que la rama de Unix, así que el análisis de abajo
        // vale para las dos.
        if (!requireDaemonForRead(ctx.connIdx,
                                  QStringLiteral("listar los directorios de Desglosar"))) {
            stopBusy();
            return;
        }
    ...
    // El listado de descendientes, por verbo tipado y en las dos plataformas.
    //
    // Había dos respaldos por shell: uno para «sin daemon» y otro para Windows. El segundo
    // no tenía motivo —el agente de Windows sirve `--dump-zfs-list-children` igual que los
    // demás— y el primero es justo lo que ya no se hace: sin agente no se opera.
    if (!requireDaemonForRead(ctx.connIdx, QStringLiteral("listar los datasets descendientes"))) {
        return;
    }
    ...
    // **Desglosar no está disponible en Windows, y aquí se dice.**
    //
    // Antes había 40 líneas de PowerShell que hacían el trabajo... y lo hacían MAL sin
    // avisar: allí los datasets se montan planos bajo la letra del pool y no admiten un punto
    // de montaje propio, así que desglosar «Z:\\des\\fotos» dejaba el dataset nuevo montado
    // en «Z:\\fotos», en la raíz de la unidad. Comprobado contra OldLau: los ficheros seguían
    // ahí pero ya no donde el usuario los tenía, y la operación decía «ok».
    //
    // El agente ya no anuncia el verbo en Windows, así que la tabla de capacidades deshabilita
    // la acción con su motivo antes de llegar aquí. Esto es la red por si se llega igualmente.
    if (isWindowsConnection(p)) {
        stopBusy();
        QMessageBox::information(
            this, QStringLiteral("ZFSMgr"),
            trk(QStringLiteral("t_desglosar_no_win_001"),
                QStringLiteral("Desglosar no está disponible en conexiones Windows: allí los "
                               "datasets se montan bajo la letra de unidad del pool y no "
                               "admiten un punto de montaje propio, así que el dataset nuevo "
                               "no quedaría donde está el directorio."),
...
void MainWindow::actionAdvancedAssemble(const DatasetSelectionContext& explicitCtx) {
    ...
    int listRc = -1;
    QString listCmd;
    // Igual que Desglosar: verbo tipado y en las dos plataformas. Aquí quedaban los mismos
    // dos respaldos —«sin daemon» y «Windows»— que el comentario de arriba dice que se
    // habían igualado; se habían igualado a medias.
    if (!requireDaemonForRead(ctx.connIdx, QStringLiteral("listar los datasets descendientes"))) {
        stopBusy();
        return;
    }
    ...
    bool allowWindowsScript = false;
    QByteArray mountStdinPayload;
    // **Por el mismo verbo que en Unix.** Aquí había 36 líneas de PowerShell que resolvían

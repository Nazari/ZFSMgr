...
ConnectionRuntimeState MainWindow::refreshConnection(const ConnectionProfile& p) {
    ...
    auto refineOsLineForRefresh = [&](const QString& currentOsLine) -> QString {
        ...
        if (uname.compare(QStringLiteral("Linux"), Qt::CaseInsensitive) == 0) {
            ...
            //
            // No es solo estética. Ese `printf` dejaba las comillas que el formato admite
            // —`NAME="Fedora Linux"`— y un espacio suelto al final cuando no hay
            // `VERSION_ID`, que es el caso de Arch y Gentoo. La versión del daemon ya
            // cuidaba las dos cosas; ahora las cuidan las dos porque son una.
            if (runSsh(profile, QStringLiteral("cat /etc/os-release"), 8000, probeOut,
                       probeErr, probeRc)
                && probeRc == 0) {

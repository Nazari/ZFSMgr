...
bool windowsAgentPending(Feature f) {
    ...
    //
    // **Esta entrada mentía, y nadie se enteraba porque nadie la consultaba.**
    //
    // Decía que Sincronizar no está en Windows. Sí está: entre máquinas se usa una tubería
    // `tar` por SSH con las dos puntas, y dentro de una misma máquina Windows va por la copia
    // propia del agente. Comprobado el 2026-08-22 de fc16 a OldLau: los ficheros llegan.
    //
    // `grep -rn "Feature::RsyncSync" src/native` no devuelve ningún consultante fuera de este
    // fichero, así que la mentira no llegaba a la interfaz —pero sí a la ayuda, que copió de
    // aquí y decía que la función no estaba disponible.
    //
    // Lo que SÍ cambia en Windows es el significado: esa tubería **no borra en el destino**,
    // así que allí Sincronizar añade y sobrescribe, pero no quita lo que sobra.
...
QString featureAgentVerb(Feature f) {
    ...
    case Feature::DirFromDirTree:       return QStringLiteral("--mutate-advanced-fromdir-prepare");

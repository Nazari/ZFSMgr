...
void MainWindow::exportPoolFromRow(int row) {
    ...
    // UNA ORDEN POR POOL.
    //
    // Este diálogo dejaba exportar varios a la vez y lo mandaba como una sola orden
    // —`zpool export tank1 tank2`—, que ZFS admite. Se pasa a una llamada por pool: es más
    // simple y uniforme con el resto de clientes, que trabajan siempre sobre uno.
    //
    // Consecuencia que hay que aceptar y que se ve en la vista previa: dejan de ser atómicas.
    // Si la segunda falla, la primera ya se exportó. Por eso el bucle PARA en el primer
    // fallo y dice cuál, en vez de seguir y dejar un resultado a medias sin explicar.
    //
    // `-a` es la excepción y no una lista: no nombra pools, significa «todos», así que sigue
    // siendo una sola orden.
    QStringList objetivos;
    if (allPoolsCb->isChecked()) {
        objetivos << QString();  // sin nombre: `zpool export -a`
        banderas.prepend(QStringLiteral("-a"));
    } else {
        objetivos << poolName;
        for (const QString& extraPool : csvArgs(poolsEd->text())) {
            const QString t = extraPool.trimmed();
            if (!t.isEmpty() && !objetivos.contains(t)) {
                objetivos << t;
            }
        }
    }
    ...
    QList<OrdenDePool> ordenes;
    QStringList previews;
    for (const QString& objetivo : objetivos) {
        QStringList argvPool;
        if (objetivo.isEmpty()) {
        ...
        if (argvPool.isEmpty()) {
            appLog(QStringLiteral("WARN"),
                   QStringLiteral("Export: argumentos no válidos para %1").arg(objetivo));
            return;
        }
        QString rawCmd = mwhelpers::cadenaDeArgv(QStringLiteral("zpool"), argvPool);
        const QStringList daemonArgv = daemonizeZpoolMutationArgs(idx, argvPool);
        ...
        ordenes.push_back({objetivo.isEmpty() ? poolName : objetivo, cmd});
        previews << QStringLiteral("[%1]\n%2")
                        .arg(sshUserHostPort(p))
                        .arg(buildSshPreviewCommand(p, cmd));
    }
    if (!confirmActionExecution(QStringLiteral("Exportar"), previews)) {
        return;
    }
    QString errorText;
    bool todoBien = true;
    for (const OrdenDePool& o : ordenes) {
        if (!executePoolCommand(idx, o.pool, QStringLiteral("Export"), o.cmd, 45000, &errorText,
                                true, false)) {
            // Se para aquí: lo ya exportado se queda exportado, y seguir dejaría un
            // resultado a medias del que nadie sabría dar cuenta.
            errorText = QStringLiteral("%1: %2").arg(o.pool, errorText);
            todoBien = false;
            break;
        }
    }
...
void MainWindow::importPoolFromRow(int row) {
    ...
    auto buildImportCommandForTarget = [&](const QString& target) -> QString {
        // Argv primero, cadena derivada de él. Esta ya era una orden POR POOL —la lambda se
        // llama con cada objetivo—, así que aquí solo cambia el sentido de la conversión.
        //
        // `parts` trae las banderas, que son muchas en importar; el nombre del pool y el
        // nombre nuevo van detrás, en ese orden, que es el que espera
        // `zpool import <origen> [<nuevo>]`.
        QStringList argvPool;
        for (int i = 1; i < parts.size(); ++i) {  // se salta el «zpool import» inicial
            argvPool << parts.at(i);
        }
        argvPool.prepend(QStringLiteral("import"));
        argvPool << target;
        if (!newNameEd->text().trimmed().isEmpty()) {
            argvPool << newNameEd->text().trimmed();
        }
        if (!extraEd->text().trimmed().isEmpty()) {
            argvPool << extraEd->text().trimmed();
        }
        QString rawCmd = mwhelpers::cadenaDeArgv(QStringLiteral("zpool"), argvPool);
...
void MainWindow::importPoolRenamingFromRow(int row) {
    ...
    auto buildImportRenameCommandForTarget = [&](const QString& target) -> QString {
        ...
        argvPool.prepend(QStringLiteral("import"));
        argvPool << target << newNameEd->text().trimmed();
        if (!extraEd->text().trimmed().isEmpty()) {
            argvPool << extraEd->text().trimmed();
        }
        QString rawCmd = mwhelpers::cadenaDeArgv(QStringLiteral("zpool"), argvPool);
    ...
    if (cmd.isEmpty()) {
        QMessageBox::warning(this, QStringLiteral("ZFSMgr"),
                             trk(QStringLiteral("t_pool_nombre_nuevo_malo001"),
                                 QStringLiteral("El nombre nuevo del pool no es válido: tiene que "
                                                "empezar por letra y solo admite letras, dígitos "
                                                "y «_-.:»."),
...
void MainWindow::scrubPoolFromRow(int row) {
    ...
    // El argv se construye PRIMERO, con `commands::pools`, y la cadena de respaldo se deriva
    // de él. Antes era al revés: se armaba la cadena y `daemonizeZpoolMutationArgs` la volvía
    // a trocear, con un corte que no respeta comillas.
    //
    // La fase —parar, pausar— la traduce el módulo a su letra, que NO es la misma en scrub
    // que en trim o initialize: ahí `-s` significa parar en uno y suspender en los otros.
    QStringList banderas;
    ...
    if (waitCb->isChecked()) {
        banderas << QStringLiteral("-w");
    }
    if (errorOnlyCb->isChecked()) {
        banderas << QStringLiteral("-e");
    }
    if (!extraEd->text().trimmed().isEmpty()) {
        banderas << extraEd->text().trimmed();
    }
...
void MainWindow::upgradePoolFromRow(int row) {
    ...
    //
    // Con «listar versiones» (`-v`), `zpool upgrade` NO nombra ningún pool: enumera las
    // características que soporta el sistema. Es una LECTURA, y por eso no pasa por
    // `commands::pools`, que exige un pool. Forzarlo habría sido distorsionar el módulo para
    // que cupiera algo que no es una operación sobre un pool.
    //
    // El otro caso sí lo es, y va por el módulo como el resto.
    QStringList argvPool;
    if (listVersionsCb->isChecked()) {
        argvPool << QStringLiteral("upgrade") << QStringLiteral("-v");
        if (!extraEd->text().trimmed().isEmpty()) {
            argvPool << extraEd->text().trimmed();
        }
    } else {
        ...
        if (!versionEd->text().trimmed().isEmpty()) {
            banderas << QStringLiteral("-V") << versionEd->text().trimmed();
        }
        if (!extraEd->text().trimmed().isEmpty()) {
            banderas << extraEd->text().trimmed();
        }
        ...
        if (argvPool.isEmpty()) {
            appLog(QStringLiteral("WARN"),
                   QStringLiteral("Upgrade: argumentos no válidos para %1").arg(poolName));
            return;
        }
    }
    QString rawCmd = mwhelpers::cadenaDeArgv(QStringLiteral("zpool"), argvPool);
...
void MainWindow::reguidPoolFromRow(int row) {
    ...
    // Argv primero, cadena derivada de él. Ver `daemonizeZpoolMutationArgs(int, QStringList)`.
    QStringList banderas;
    if (!guidEd->text().trimmed().isEmpty()) {
        banderas << QStringLiteral("-g") << guidEd->text().trimmed();
    }
    if (!extraEd->text().trimmed().isEmpty()) {
        banderas << extraEd->text().trimmed();
    }
...
void MainWindow::clearPoolFromRow(int row) {
    ...
    // Argv primero; la cadena de respaldo se deriva de él. El módulo garantiza el
    // orden que pide zpool —banderas, pool, discos—, que es donde estuvo el fallo de
    // «invalid character '/' in pool name» por poner los discos delante.
    QStringList banderas;
    QStringList discos;
    if (powerCb->isChecked()) {
        banderas << QStringLiteral("--power");
    }
    if (!extraEd->text().trimmed().isEmpty()) {
        banderas << extraEd->text().trimmed();
    }
...
void MainWindow::syncPoolFromRow(int row) {
    ...
    // Una orden por pool, como en Exportar. `zpool sync` SIN pool significa «todos», así que
    // ese caso sigue siendo una sola orden: no es una lista vacía, es otra cosa.
    QStringList objetivos;
    if (allPoolsCb->isChecked()) {
        objetivos << QString();
    } else {
    ...
    for (const QString& objetivo : objetivos) {
        ...
        if (argvPool.isEmpty()) {
            appLog(QStringLiteral("WARN"),
                   QStringLiteral("Sync: argumentos no válidos para %1").arg(objetivo));
            return;
        }
        QString rawCmd = mwhelpers::cadenaDeArgv(QStringLiteral("zpool"), argvPool);
        const QStringList daemonArgv = daemonizeZpoolMutationArgs(idx, argvPool);
    ...
    if (!confirmActionExecution(QStringLiteral("Sync"), previews)) {
        return;
    }
    QString errorText;
    bool todoBien = true;
    for (const OrdenDePool& o : ordenes) {
        if (!executePoolCommand(idx, o.pool, QStringLiteral("Sync"), o.cmd, 45000, &errorText,
                                true, true)) {
            errorText = QStringLiteral("%1: %2").arg(o.pool, errorText);
            todoBien = false;
            break;
...
void MainWindow::trimPoolFromRow(int row) {
    ...
    if (secureCb->isChecked()) {
        banderas << QStringLiteral("-d");
    }
    if (!rateEd->text().trimmed().isEmpty()) {
        banderas << QStringLiteral("-r") << (rateEd->text().trimmed());
    }
    if (!extraEd->text().trimmed().isEmpty()) {
        banderas << extraEd->text().trimmed();
    }

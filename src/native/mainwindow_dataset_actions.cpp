...
// Tercera copia retirada: ahora es `currentLocalMachineUid()` de connectionstore.h, que es
// la única que lee el registro en Windows. Esta caía a `QSysInfo` allí, así que comparaba
// contra un identificador distinto del que se había guardado.
...
DaemonMutationPlan daemonMutationPlanForCommand(const QString& rawCmd, const QByteArray& stdinPayload = {}) {
    ...
    // Aquí se detectaban las secuencias de `zfs allow` unidas por «; » y se volvían a
    // trocear para armar el lote. Ya no hace falta: quien las produce —la ficha de
    // permisos— compone el argv de cada orden y pasa el lote ya codificado, así que
    // ninguna cadena tiene que deshacerse a este lado.
...
bool MainWindow::executePendingDatasetRenameDraft(const PendingDatasetRenameDraft& draft,
    ...
    // Argv primero, cadena derivada de él. Y con ello llega la regla de
    // `commands::datasets`: un nombre nuevo SIN barra conserva el padre, en vez de acabar en
    // un «cannot create 'x': missing dataset name» que no dice qué hacer.
    const QStringList argvRen =
        mwhelpers::argvRenombrarDataset(draft.sourceName.trimmed(), draft.targetName.trimmed());
    if (argvRen.isEmpty()) {
        appLog(QStringLiteral("WARN"),
               QStringLiteral("Renombrar: nombre no válido para %1").arg(draft.sourceName.trimmed()));
        return false;
    }
    const QString renameRawCmd = mwhelpers::cadenaDeArgv(QStringLiteral("zfs"), argvRen);
    QString renameExecCmd = renameRawCmd;
    QStringList renameArgv;
    if (!isWindowsConnection(p)) {
        renameArgv = daemonizeZfsMutationArgs(draft.connIdx, argvRen);
...
bool MainWindow::umountDataset(const QString& side, const DatasetSelectionContext& ctx) {
    ...
    // ¿Hay descendientes montados? Se contesta con la lista de montajes que ya sabe dar el
    // agente, en vez de con un guion por plataforma ejecutado por SSH solo para decir sí o no.
    QString out;
    QString err;
    int rc = -1;
    const ConnectionProfile p = m_conns.profiles[ctx.connIdx];
    if (!requireDaemonForRead(ctx.connIdx, QStringLiteral("saber si hay hijos montados"))) {
        return false;
    }

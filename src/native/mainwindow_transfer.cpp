...
bool MainWindow::queueDatasetAction(const QString& side,
    ...
    QString errorText;
    if (!runShellActionNow(draft, &errorText)) {
...
bool MainWindow::runShellActionNow(const PendingShellActionDraft& draft, QString* errorOut) {
    // Antes esto ENCOLABA. Ahora ejecuta.
    //
    // La lista de cambios pendientes se retira: una acción de menú se hace al pulsarla, como
    // en cualquier otro sitio. Lo que se va con ella no es solo la espera, son tres cosas que
    // solo existían por diferir:
    //
    //   - La advertencia de «esta orden la construyó una versión anterior»: una orden que
    //     esperaba días en disco podía ser más vieja que el arreglo del código que la
    //     construye. Ejecutando al instante, quien construye y quien ejecuta son el mismo
    //     binario, siempre.
    //   - La casilla «Activa» y la reactivación de una entrada ya ejecutada: pedir dos veces
    //     lo mismo es, sencillamente, hacerlo dos veces.
    //   - El rechazo por duplicado, que obligaba a ir a la lista a quitar la entrada antes
    //     de poder repetir la acción.
    //
    // El registro del rechazo se queda AQUÍ y no en los llamantes: son veinte y ninguno lo
    // hacía, así que un fallo salía en un diálogo y el log no decía nada.
    auto fail = [this, errorOut, &draft](const QString& text) {
        if (errorOut) {
            *errorOut = text;
        }
        appLog(QStringLiteral("WARN"),
               QStringLiteral("[accion] no se ejecutó «%1»: %2")
                   .arg(draft.displayLabel.trimmed().isEmpty() ? draft.command.trimmed()
                                                               : draft.displayLabel.trimmed(),
                        text));
        return false;
    };
    if (draft.command.trimmed().isEmpty() && draft.datasetActionName.trimmed().isEmpty()
    ...
    if (!executeShellActionDraft(aEjecutar)) {
        ...
        return fail(QStringLiteral("La acción falló; revise el registro."));
    }
    return true;
}
...
void MainWindow::actionCopySnapshot() {
...
#ifdef Q_OS_WIN
    // Desde un cliente WINDOWS, si no se pudo montar el camino por AGENTE, aquí se para.
    //
    // La versión anterior de esta guarda solo actuaba con el pipeline vacío, y el caso
    // que fallaba tiene pipeline: el «daemon-a-daemon» SÍNCRONO, que pese al nombre se
    // ejecuta como una cadena de shell POSIX construida con shSingleQuote y lanzada por
    // la máquina local, con sudo en el origen. Dejarla pasar es lo que producía «sudo: 2
    // incorrect password attempts» con la contraseña correcta, y me costó cuatro rondas
    // de diagnóstico apuntando a las credenciales en vez de al camino.
    //
    // Desde Windows la copia va por RPC a los dos daemons o no va: es la decisión de
    // diseño de las fases 3 y 5, y aquí se hace valer.
    {
        QMessageBox::warning(
            this, QStringLiteral("ZFSMgr"),
            trk(QStringLiteral("t_copy_needs_daemon001"),
                QStringLiteral("No se pudo preparar la copia.\n\n"
                               "Desde Windows, copiar entre máquinas se hace siempre a través "
                               "del agente, y no se pudo montar ese camino. Compruebe en la "
                               "ficha de ambas conexiones que el agente está instalado y "
                               "activo, y que la versión de API coincide."),
    ...
    // **Entre máquinas, la copia va por los dos daemons o no va.**
    //
    // Aquí había tres respaldos, y los tres construían SHELL. El primero se llamaba
    // «daemon a daemon» pero se ejecutaba como una cadena POSIX lanzada desde este equipo
    // con sudo en el origen; los otros dos encadenaban `zfs send | ssh … zfs recv`, uno
    // directo entre las dos máquinas y otro pasando los datos por el host local.
    //
    // Se van los tres. Motivos, por orden de peso:
    //
    // 1. El trabajo por daemon —el camino preferente desde la fase 5— ya cubre todos esos
    //    casos, y encima sobrevive a que se cierre la ventana.
    // 2. El primero resolvía la dirección del destino con el `host` del perfil, que cuando
    //    el destino es la conexión Local vale «localhost»: desde una máquina remota eso
    //    apunta al propio origen. Es el mismo fallo que se acaba de quitar del intérprete.
    // 3. Son shell, que es lo que el endurecimiento RPC está retirando; mantenerlos
    //    obligaba a conservar para siempre la construcción de órdenes de shell.
    //
    // Lo que queda, si el camino por trabajo no se pudo montar, es DECIRLO. Un error
    // entendible vale más que una copia que sale por un camino peor sin avisar.
    if (!sameConnection) {
        QMessageBox::warning(
            this, QStringLiteral("ZFSMgr"),
            trk(QStringLiteral("t_copy_needs_jobs001"),
                QStringLiteral("No se pudo preparar la copia entre %1 y %2.\n\n"
                    ...
                    .arg(sp.name, dp.name),
                QStringLiteral("Could not prepare the copy between %1 and %2.\n\n"
                               "Copying between machines uses an agent job on both ends, and "
    ...
    // En la MISMA máquina no hace falta socket ni trabajo: el daemon encadena send y recv
...
void MainWindow::actionCloneSnapshot() {
    ...
    QStringList banderas;
    if (pChk->isChecked()) {
        banderas << QStringLiteral("-p");
    }
    if (uChk->isChecked()) {
        banderas << QStringLiteral("-u");
    }
...
void MainWindow::actionDiffSnapshot() {
    ...
    // Sin agente no se compara: aquí caía a `zfs diff` por SSH con sudo.
    if (!requireDaemonForRead(src.connIdx, QStringLiteral("comparar dos instantáneas"))) {
        QMessageBox::warning(
            this, QStringLiteral("ZFSMgr"),
...
void MainWindow::actionLevelSnapshot() {
    ...
    if (!sameConnection) {
        QMessageBox::warning(
            this, QStringLiteral("ZFSMgr"),
            trk(QStringLiteral("t_level_needs_jobs001"),
                QStringLiteral("No se pudo preparar el nivelado entre %1 y %2.\n\n"
                               "Nivelar entre máquinas se hace con un trabajo del agente en "
                               "ambos extremos, y ese camino no se pudo montar. Compruebe en la "
                               "ficha de las dos conexiones que el agente está instalado, "
                               "activo y con la versión de API al día.")
                    .arg(sp.name, dp.name),
                QStringLiteral("Could not prepare the level between %1 and %2.\n\n"
                               "Levelling between machines uses an agent job on both ends, and "
                               "that path could not be set up. Check on both connection cards "
                               "that the agent is installed, running and with an up-to-date API "
                               "version.")
                    .arg(sp.name, dp.name),
    ...
    if (typedPipe.isEmpty()) {
        QMessageBox::warning(
            this, QStringLiteral("ZFSMgr"),
            trk(QStringLiteral("t_level_needs_pipe001"),
                QStringLiteral("No se pudo preparar el nivelado en %1.\n\n"
                               "Nivelar dentro de una misma máquina lo hace el agente. "
                               "Compruebe que está instalado, activo y al día.")
                    .arg(sp.name),
...
void MainWindow::actionSyncDatasets() {
    ...
    // ¿Tiene esa máquina esta herramienta? Del estado que ya llenó el refresco.
    //
    // Aquí se sondeaba por SSH con `command -v` o `Get-Command`, y se llamaba CUATRO veces
    // —zstd y gzip, en los dos extremos— cada vez que se abría el diálogo. El agente ya
    // contesta qué le falta en `--dump-tool-availability`, y el refresco lo guarda.
    auto remoteHasTool = [this](int connIdx, const QString& tool) -> bool {
        if (connIdx < 0 || connIdx >= m_conns.states.size() || tool.trimmed().isEmpty()) {
            return false;
        }
        return !m_conns.states[connIdx].missingUnixCommands.contains(tool.trimmed());
    };
    ...
    if (srcRootMounted && dstRootMounted) {
        ...
        auto buildRsyncSyncCommand = [&](bool useDelete, bool dryRun) -> QString {
            ...
            // Aquí se armaba un `rsync` de shell con una sonda de opciones por delante,
            // y se ejecutaba con sudo por SSH. Sin agente ya no se sincroniza: la guarda
            // está antes de abrir el diálogo, así que llegar aquí sería un fallo de
            // composición del verbo, no falta de daemon.
            return QString();
        };
        // La comprobación va ANTES del diálogo: preguntarle al usuario las opciones de una
        // sincronización que no se va a poder hacer es hacerle perder el tiempo, y peor,
        // enseñarle una vista previa de una orden que no es la que se ejecutaría.
        //
        // Las DOS puntas: sincronizar compara y copia entre dos máquinas, y sin agente en
        // cualquiera de ellas no hay camino.
        if (!requireDaemonForMutation(src.connIdx, QStringLiteral("sincronizar"))) {
            return;
        }
        if (!sameConnection && !requireDaemonForMutation(dst.connIdx,
    ...
    if (isWindowsConnection(sp) || isWindowsConnection(dp)) {
        ...
        auto buildSubdatasetRsyncCommand = [&](bool useDelete, bool dryRun) -> QString {
            ...
            // Ídem: el `set -e; … && …` de rsync por shell ya no está.
            return QString();
        };
        if (!requireDaemonForMutation(src.connIdx, QStringLiteral("sincronizar subdatasets"))) {
            return;
        }
        if (!sameConnection && !requireDaemonForMutation(dst.connIdx,
                                                         QStringLiteral("sincronizar subdatasets"))) {
            return;
        }
        bool useDelete = true;

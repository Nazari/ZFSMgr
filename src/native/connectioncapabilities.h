...
enum class Feature {
    ...
    DirToDir,              // Hacia Dir
    DirFromDirTree,        // Desde Dir SIN tubería: árbol entre daemons
...
struct Platform {
    ...
    // Capacidades declaradas por el propio agente. El daemon las publica en `--health`
    // como `CAPS=`, y mandan sobre la tabla estática de abajo: la tabla es una suposición
    // del cliente y su respuesta es un hecho. Con un agente viejo llega vacío y entonces
    // sí decide la tabla.

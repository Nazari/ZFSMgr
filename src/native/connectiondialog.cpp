...
// El descriptor fijo por el que el hijo recibe la contraseña.
//
// Hace falta un número CONOCIDO porque la opción de sshpass lo lleva dentro («-d3»), y el
// argumento se construye aquí, en el padre, antes de saber qué descriptor le tocará al
// hijo. Con QProcess la pieza que lo permite es el modificador de proceso hijo, que corre
// ya en el hijo y justo antes del exec: ahí se duplica el descriptor real sobre el 3.
constexpr int kFdDeLaClave = 3;

// Prepara el proceso para que la contraseña llegue por descriptor y NO por la línea de
...
QStringList preparaClavePorDescriptor(QProcess& proc,
    ...
    proc.setChildProcessModifier([origen]() {
        // Corre en el hijo, entre el fork y el exec. `dup2` deja el descriptor nuevo sin
        // la marca de cierre-al-exec, que es justo lo que hace falta para que sobreviva.
        if (origen != kFdDeLaClave) {
            ::dup2(origen, kFdDeLaClave);
        }
    });
    return QStringList{QStringLiteral("-d%1").arg(kFdDeLaClave), QStringLiteral("ssh")};
#else
    Q_UNUSED(proc);
    Q_UNUSED(secreto);
    return {};  // en Windows no hay sshpass
#endif
}
...
bool ConnectionDialog::testSshConnection(const ConnectionProfile& p, QString& detail) const {
    ...
    QProcess proc;
    if (usingSshpass) {
        const QStringList prefijo = preparaClavePorDescriptor(proc, secreto);
        if (prefijo.isEmpty()) {
            // Sin descriptor no hay forma seria de darle la contraseña: se lanza ssh a
            // secas y que falle por credenciales. Publicarla en el argv no es una opción.
...
bool ConnectionDialog::detectSshPlatform(const ConnectionProfile& p,
    ...
    if (runSshProbe(p, QStringLiteral("uname -s"), 8000, out, err)) {
        ...
        if (uname.compare(QStringLiteral("Linux"), Qt::CaseInsensitive) == 0) {
            ...
            QString lOut;
            QString lErr;
            // El fichero viaja entero y se interpreta aquí, con la función de la capa base
            // que comparten esta sonda, el refresco de la ventana y el propio daemon. Antes
            // eran tres formas distintas de leer las mismas dos líneas, y solo la del daemon

...
QStringList aLista(const std::vector<std::string>& v) {
    QStringList out;
    for (const std::string& s : v) {
        out << QString::fromStdString(s);
    }
...
QStringList argvPool(zfsmgr::commands::pools::Operacion op, const QString& pool,
    ...
    for (const QString& x : discos) {
        d.push_back(x.toStdString());
    }
    QStringList out;
    for (const std::string& x :
         zfsmgr::commands::pools::argv(op, pool.toStdString(), fase, b, d)) {
        out << QString::fromStdString(x);
    }
    return out;
}

QStringList argvQt(const std::vector<std::string>& argv) {
    QStringList out;
    out.reserve(static_cast<int>(argv.size()));
...
QString cadenaDeArgv(const QString& programa, const QStringList& argv) {
    QStringList partes;
    partes << programa;
    for (const QString& a : argv) {
        // Se entrecomilla TODO, incluidas las banderas: `'-w'` funciona igual en un shell y
        // así no hay que decidir caso por caso qué necesita comillas y qué no. Decidirlo era
        // exactamente lo que se hacía mal —solo se entrecomillaba el nombre del pool—.
        partes << shSingleQuote(a);
    }

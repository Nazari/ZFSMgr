                           ...
                           mwhelpers::argvQt(zfsmgr::commands::peticiones::propiedadesConcretas(
                               [&wantedProps]() {
                                   ...
                                   for (const QString& w : wantedProps) {
                                       v.push_back(w.toStdString());
                                   }
                                   return v;
                               }(),
                               trimmedObject.toStdString())),
                           20000, out, err, rc)
        && rc == 0;
    // Re-look up after the yield: rebuildConnInfoFor() replaces the whole ConnInfo,
...
// La variante que recibe el argv YA CONSTRUIDO, sin cadena de por medio.
//
// La de abajo toma una cadena de shell y la vuelve a trocear. Ese viaje —argv → cadena →
// argv— es el que el plan técnico señalaba como origen de fallos: el corte no respeta
// comillas, así que un nombre con `;`, `&` o `|` dentro trunca los argumentos.
//
// La cadena sigue haciendo falta, pero solo para el camino de RESPALDO por SSH cuando no hay
// daemon. Con esta variante se construye primero el argv —desde `commands::pools`, con sus
// reglas— y la cadena se deriva de él, que es el sentido que sí sobrevive.
QStringList MainWindow::daemonizeZpoolMutationArgs(int connIdx, const QStringList& argv) const {
    ...
    const QString op = argv.first().trimmed();
    if (!isAllowedGenericZpoolMutationOpClient(op)) {
        return {};
    }
    // Importar por RPC solo si el daemon de esa máquina lo sirve; si no, por el camino
    // clásico. Es la misma condición que la variante de cadena.
...
// no respeta comillas, y la cadena solo hace falta para el camino de respaldo por SSH.
QStringList MainWindow::daemonizeZfsMutationArgs(int connIdx, const QStringList& argv) const {
    if (connIdx < 0 || connIdx >= m_conns.profiles.size() || connIdx >= m_conns.states.size()) {
        return {};
    }
    if (argv.isEmpty()) {
...
QStringList MainWindow::daemonizeZfsAllowBatchArgs(int connIdx,
                                                   const QList<QStringList>& ordenes) const {
    if (connIdx < 0 || connIdx >= m_conns.profiles.size() || connIdx >= m_conns.states.size()) {
        return {};
    }
    if (ordenes.isEmpty()) {
    ...
    // El envoltorio es una lista de listas: cada orden se codifica por su cuenta y el lote
    // vuelve a codificarse entero. Así un nombre de usuario con un espacio —o un dataset con
    // una comilla— llega intacto, porque en ningún punto del viaje hay una cadena que
    // alguien tenga que volver a partir.
    QStringList lote;
    for (const QStringList& argv : ordenes) {
        if (argv.size() < 2) {
            return {};
        }
        const QString op = argv.first().trimmed().toLower();
...
QStringList MainWindow::daemonizeLocalSendRecvArgs(int connIdx,
    ...
    auto toArgv = [](const QString& rawCmd, const QString& expectOp) -> QStringList {
    ...
    const QStringList sendArgv = toArgv(sendRawCmd, QStringLiteral("send"));
    const QStringList recvArgv = toArgv(recvRawCmd, QStringLiteral("recv"));
...
QStringList MainWindow::daemonizeRsyncSyncArgs(int connIdx,
    ...
    // La carga la arma la capa base. El orden de los campos es un contrato del DAEMON, y
    // estaba escrito tres veces —aquí, en el intérprete y en el servidor web—; tres sitios
    // donde equivocarse por separado el día que ese contrato cambie.
...
bool MainWindow::fetchConnectionProbeOutput(int sourceConnIdx,
    ...
    QString err;
    int rc = -1;
    const bool ok = runSsh(src, remoteCmd, timeoutMs, out, err, rc, nullptr, nullptr, nullptr,
                           stdinPayload);
...
bool MainWindow::ensureDatasetSnapshotHoldsLoaded(int connIdx, const QString& poolName, const QString& objectName) {
    ...
    QString err;
    int rc = -1;
    // Los holds, por verbo tipado. Aquí se armaba `zfs holds -H` y se ejecutaba con sudo por
    // SSH, sin mirar siquiera si había agente.
    if (!requireDaemonForRead(connIdx, QStringLiteral("leer los holds"))) {
        return false;
    }
    const bool holdsOk =
...
bool MainWindow::requireDaemonForMutation(int connIdx, const QString& what) {
    if (requireDaemonForRead(connIdx, what)) {
        return true;
    }
    const QString nombre = m_conns.profiles.value(connIdx).name.trimmed();
    QMessageBox::warning(
        this, QStringLiteral("ZFSMgr"),
        trk(QStringLiteral("t_mutacion_sin_daemon_001"),
            QStringLiteral("No se puede %1 en «%2»: esa conexión no tiene un agente "
...
bool MainWindow::requireDaemonForRead(const QString& connName,
    ...
    if (!st.daemonInstalled) {
        // **Sin refrescar no es lo mismo que sin agente.** El estado empieza vacío y lo llena
        // el refresco; antes de él, `daemonInstalled` es falso porque no se ha mirado, no
        // porque se haya mirado y no esté. Decir «reinstale el daemon» ahí manda al usuario a
        // arreglar algo que no está roto.
        //
        // Se distinguen por `daemonDetail`: el refresco lo rellena siempre, con el motivo o
        // con la versión. Vacío significa que nadie ha preguntado todavía.
        reason = st.daemonDetail.trimmed().isEmpty()
                     ? QStringLiteral("la conexión aún no se ha refrescado")
                     : QStringLiteral("el agente no está instalado");
    ...
    const_cast<MainWindow*>(this)->appLog(
        QStringLiteral("WARN"),
        QStringLiteral("%1: no se puede %2 porque %3.%4")
            .arg(connName, what, reason,
                 st.daemonDetail.trimmed().isEmpty()
                     ? QStringLiteral(" Refresque la conexión.")
                     : QStringLiteral(" Reinstale el daemon desde el menú de la conexión.")));
    return false;
}
...
QStringList MainWindow::poolContextMenuLabelsForTest(const QString& poolName, bool bottom) const {
    ...
    // **Se devuelve lo que el estado dice, no una lista fija.**
    //
    // Aquí había un `Q_UNUSED(menuState)` seguido de todas las etiquetas a pelo: el estado
    // se calculaba justo encima y se tiraba. Consecuencia medida: las capturas de la ayuda
    // salían idénticas para un pool importado y para uno sin importar, y el mismo menú
    // ofrecía a la vez «Importar» y «Exportar», que es un menú que no existe en la
    // aplicación. Dos imágenes distintas en la ayuda mostrando lo mismo, y lo mismo era
    // falso.
    QStringList out;
    ...
    if (menuState.canSync)       { out.push_back(QStringLiteral("Sync")); }
    if (menuState.canScrub)      { out.push_back(QStringLiteral("Scrub")); }
    if (menuState.canUpgrade)    { out.push_back(QStringLiteral("Upgrade")); }
    if (menuState.canReguid)     { out.push_back(QStringLiteral("Reguid")); }
    if (menuState.canTrim)       { out.push_back(QStringLiteral("Trim")); }
    if (menuState.canInitialize) { out.push_back(QStringLiteral("Initialize")); }
    if (menuState.canClear)      { out.push_back(QStringLiteral("Clear")); }
    if (menuState.canDestroy)    { out.push_back(QStringLiteral("Destroy")); }
    // Este no depende del estado del pool: es una preferencia de visualización.
    out.push_back(QStringLiteral("Mostrar Datasets programados"));
    return out;
}

// Identificador estable de una conexión, y su vuelta.
//
// Vivían en el fichero de la lista de pendientes y se llamaban `connectionKeyByIndex` /
// `connectionIndexByKey`, pero no tenían nada que ver con ella: son «id de conexión ↔ índice», y
// los usa «Desde Dir» para recordar de qué máquina venía cada origen. Al retirar la lista se
// quedaban sin casa, así que pasan aquí con el nombre de lo que hacen.
//
// El índice NO sirve para recordar nada entre sesiones: cambia al añadir o borrar
// conexiones. Por eso se guarda la clave.
QString MainWindow::connectionKeyByIndex(int connIdx) const {
    if (connIdx < 0 || connIdx >= m_conns.profiles.size()) {
        return QString();
    }
...
int MainWindow::connectionIndexByKey(const QString& key) const {
    const QString wanted = key.trimmed().toLower();
    if (wanted.isEmpty()) {
        return -1;
    }
    for (int i = 0; i < m_conns.profiles.size(); ++i) {
        if (connectionKeyByIndex(i) == wanted) {
            return i;
        }
    }
    return -1;

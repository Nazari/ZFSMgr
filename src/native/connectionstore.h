...
// El identificador de ESTA máquina, uno solo para toda la aplicación.
//
// Había tres implementaciones distintas —aquí, en main.cpp y en mainwindow_dataset_actions—
// y no devolvían lo mismo: solo esta lee el `MachineGuid` del registro en Windows; las otras
// dos caían a `QSysInfo::machineUniqueId()`, que es otra cadena. Como ese valor se ESCRIBE
// en el perfil local desde un sitio y se COMPARA desde otro, en Windows podían no casar y la
// aplicación no reconocía su propia máquina.
//
// Esta es la buena: registro en Windows, IOPlatformUUID en macOS y `QSysInfo` como último
// recurso, en minúsculas y cacheada. Coincide con `detectMachineUuid()` del daemon.

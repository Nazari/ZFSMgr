...
- `--mutate-advanced-fromdir` (tipado, **solo CLI**): completa el juego junto a breakdown/assemble/todir. Monta el dataset, resuelve el mountpoint, crea el subdirectorio y extrae el tar entrante, todo con execvp. No se sirve por RPC porque lee el stream por stdin y el canal RPC no lo tiene. Valida el subdirectorio relativo (rechaza rutas absolutas y `..`), cosa que el script de shell anterior no hacía.
- **`--mutate-shell-generic` ya NO se sirve por RPC**: ejecuta shell arbitrario como root, así que exponerlo permitía a cualquier cliente con el certificado mTLS saltarse todas las whitelists `--mutate-*`. Se conserva únicamente como comando CLI, que es como la GUI lo invoca siempre (SSH + sudo, donde el llamante ya es root y no gana privilegio alguno). Verificado con cliente mTLS real: el RPC responde `unknown command` y no ejecuta nada.

    ...
    def _run_assemble_psrp_command(
        ...
        def _build_child_script(dataset_name: str, mountpoint_hint: str, child_name: str) -> str:
            ...
            return (
                ...
                "  if ($free2) { "
                "    try { zfs set (\"driveletter=\" + $free2) $child | Out-Null } catch {}; "
                "    try { zfs mount $child | Out-Null } catch {}; "
                "    $cand2 = ($free2 + ':\\'); "
                "    if (Test-Path -LiteralPath $cand2) { $childActive = $cand2; $childTempDrive = $free2 } "
                "  } "
                "} "
                "if (-not ($childActive -and (Test-Path -LiteralPath $childActive))) { throw \"[ASSEMBLE][ERROR] cannot access child mountpoint for $child\" }; "
                "$copiedStage = $false; "
                "if (Get-Command robocopy -ErrorAction SilentlyContinue) { "

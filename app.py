    ...
    def _run_assemble_psrp_command(
        ...
        def _build_root_prepare_script(dataset_name: str, mountpoint_hint: str) -> str:
            ...
            return (
                ...
                "  New-Item -ItemType Directory -Path $tmpMp -Force | Out-Null; "
                "  try { zfs unmount $dataset *> $null } catch {}; "
                "  try { zfs set (\"mountpoint=\" + $tmpMp) $dataset | Out-Null } catch { throw (\"[ASSEMBLE][ROOT] set mountpoint failed: \" + $_.Exception.Message) }; "
                "  try { zfs mount $dataset | Out-Null } catch { throw (\"[ASSEMBLE][ROOT] mount failed after set mountpoint: \" + $_.Exception.Message) }; "
                "  $mp = $tmpMp; "
                "  $datasetTempMp = $tmpMp; "
                "} "
                "if ([string]::IsNullOrWhiteSpace($mp) -or -not (Test-Path -LiteralPath $mp)) { "

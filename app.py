    ...
    def _run_assemble_psrp_command(
        ...
        def _build_child_script(dataset_name: str, mountpoint_hint: str, child_name: str) -> str:
            ...
            return (
                ...
                "  New-Item -ItemType Directory -Path $tmpMp -Force | Out-Null; "
                "  try { zfs unmount $dataset *> $null } catch {}; "
                "  try { zfs set (\"mountpoint=\" + $tmpMp) $dataset | Out-Null } catch { $diag += ('set dataset mountpoint failed: ' + $_.Exception.Message) }; "
                "  try { zfs mount $dataset | Out-Null } catch { $diag += ('mount dataset temp failed: ' + $_.Exception.Message) }; "
                "  $mp = $tmpMp; "
                "  $datasetTempMp = $tmpMp; "
                "} "
                "if (-not (Test-Path -LiteralPath $mp)) { "
                ...
                "$childActive=''; "
                "try { zfs unmount $child *> $null } catch {}; "
                "try { zfs set (\"mountpoint=\" + $childTmp) $child | Out-Null } catch { $diag += ('set child mountpoint failed: ' + $_.Exception.Message) }; "
                ...
                "  } "
                "} catch { $childActive='' }; "
                "if (-not $childActive) { $childActive = $childTmp }; "
                "if (-not (Test-Path -LiteralPath $childActive)) { $childActive = $childTmp }; "
                "if (-not (Test-Path -LiteralPath $childActive)) { "
                ...
                "} "
                "try { zfs unmount $child *> $null } catch {}; "
                "if ($childMpOrig -and $childMpOrig -ne 'none') { "
                "  try { zfs set (\"mountpoint=\" + $childMpOrig) $child | Out-Null } catch {} "
                "} else { "
                "  try { zfs inherit mountpoint $child | Out-Null } catch {} "
                "}; "
                "zfs destroy -r $child | Out-Null; "
                "New-Item -ItemType Directory -Path $dest -Force | Out-Null; "
                "$copiedDest = $false; "

...
class App(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        datasets_main.columnconfigure(1, weight=0, minsize=345)
    ...
    def _run_assemble_psrp_command(
        ...
        def _build_child_script(dataset_name: str, mountpoint_hint: str, child_name: str) -> str:
            ...
            return (
                ...
                "  } "
                "} catch { $mp='' }; "
                "if ($mp -and ($mp -match '^[A-Za-z]:[\\\\/].*')) { $mpLooksDrive = $true }; "
                "if ($mp -and ($mpLooksDrive) -and (-not (Test-Path -LiteralPath $mp))) { $mp = ($mp -replace '/', '\\') }; "
                "if (-not ($mp -and ((Test-Path -LiteralPath $mp) -or $mpLooksDrive))) { "
                ...
                "if ($mp -and ($mpLooksDrive) -and (-not (Test-Path -LiteralPath $mp))) { $mp = ($mp -replace '/', '\\') }; "
                "if (-not ($mp -and ((Test-Path -LiteralPath $mp) -or $mpLooksDrive))) { "
                "  if ($mpHint -and (Test-Path -LiteralPath $mpHint)) { $mp = $mpHint } "
                "} "
                "if (-not ($mp -and ((Test-Path -LiteralPath $mp) -or $mpLooksDrive))) { "
                "  $used = @{}; "
                ...
                "  if ($free) { "
                "    try { zfs set (\"driveletter=\" + $free) $dataset | Out-Null } catch { $diag += ('set driveletter failed: ' + $_.Exception.Message) }; "
                "    try { zfs mount $dataset | Out-Null } catch { $diag += ('mount dataset failed: ' + $_.Exception.Message) }; "
                "    $cand = ($free + ':\\'); "
                "    for ($i=0; $i -lt 10; $i++) { if (Test-Path -LiteralPath $cand) { break }; Start-Sleep -Milliseconds 250 }; "
                "    if (Test-Path -LiteralPath $cand) { $mp = $cand; $datasetTempDrive = $free } else { $diag += ('drive not visible after mount: ' + $cand) } "
                "  } "
                "} "
                "if (-not ($mp -and ((Test-Path -LiteralPath $mp) -or $mpLooksDrive))) { "
                "  try { $diag += ('mounted=' + ((zfs get -H -o value mounted $dataset).Trim())) } catch {}; "
                "  try { $diag += ('mountpoint=' + ((zfs get -H -o value mountpoint $dataset).Trim())) } catch {}; "
                "  try { $diag += ('driveletter=' + ((zfs get -H -o value driveletter $dataset).Trim())) } catch {}; "
                "  try { $diag += ('zfs mount rows: ' + ((zfs mount | Out-String).Trim() -replace \"`r?`n\", ' | ')) } catch {}; "
                "  Write-Output ('[ASSEMBLE][DIAG] ' + ($diag -join ' || ')); "
                ...
                "$childDriveOrig=''; "
                "try { $childDriveOrig = (zfs get -H -o value driveletter $child).Trim() } catch { $childDriveOrig='' }; "
                "$childTempDrive=''; "
                "$childActive=''; "
                "$childLooksDrive=$false; "
                "try { "
                "  $mountedRows = zfs mount; "
                "  foreach ($line in $mountedRows) { "
                "    $parts = ($line -split '\\s+'); "
                "    if ($parts.Length -ge 2 -and $parts[0] -eq $child) { $childActive = $parts[1]; break } "
                "  } "
                "} catch { $childActive='' }; "
                "if ($childActive -and ($childActive -match '^[A-Za-z]:[\\\\/].*')) { $childLooksDrive = $true }; "
                "if ($childActive -and ($childLooksDrive) -and (-not (Test-Path -LiteralPath $childActive))) { $childActive = ($childActive -replace '/', '\\') }; "
                "if (-not ($childActive -and ((Test-Path -LiteralPath $childActive) -or $childLooksDrive))) { "
                "  try { zfs mount $child | Out-Null } catch {}; "
                "  try { "
                "    $mountedRows = zfs mount; "
                "    foreach ($line in $mountedRows) { "
                "      $parts = ($line -split '\\s+'); "
                ...
                "if (-not ($childActive -and ((Test-Path -LiteralPath $childActive) -or $childLooksDrive))) { "
                "  $used2 = @{}; "
                "  Get-PSDrive -PSProvider FileSystem -ErrorAction SilentlyContinue | ForEach-Object { $used2[$_.Name.ToUpper()] = $true }; "
                "  $letters2 = @('Z','Y','X','W','V','U','T','S','R','Q','P','O','N','M','L','K','J','I','H','G','F','E','D'); "
                "  $free2 = $null; "
                "  foreach ($l in $letters2) { if (-not $used2.ContainsKey($l)) { $free2 = $l; break } }; "
                "  if ($free2) { "
                "    try { zfs set (\"driveletter=\" + $free2) $child | Out-Null } catch { $diag += ('set child driveletter failed: ' + $_.Exception.Message) }; "
                "    try { zfs mount $child | Out-Null } catch { $diag += ('mount child failed: ' + $_.Exception.Message) }; "
                "    $cand2 = ($free2 + ':\\'); "
                "    for ($j=0; $j -lt 10; $j++) { if (Test-Path -LiteralPath $cand2) { break }; Start-Sleep -Milliseconds 250 }; "
                "    if (Test-Path -LiteralPath $cand2) { $childActive = $cand2; $childTempDrive = $free2 } else { $diag += ('child drive not visible after mount: ' + $cand2) } "
                "  } "
                "} "
                "if (-not ($childActive -and ((Test-Path -LiteralPath $childActive) -or $childLooksDrive))) { "
                "  try { $diag += ('child mounted=' + ((zfs get -H -o value mounted $child).Trim())) } catch {}; "
                "  try { $diag += ('child mountpoint=' + ((zfs get -H -o value mountpoint $child).Trim())) } catch {}; "
                "  try { $diag += ('child driveletter=' + ((zfs get -H -o value driveletter $child).Trim())) } catch {}; "
                "  Write-Output ('[ASSEMBLE][DIAG] ' + ($diag -join ' || ')); "
                "  throw \"[ASSEMBLE][ERROR] cannot access child mountpoint for $child\" "
                "}; "
                "$copiedStage = $false; "
                "if (Get-Command robocopy -ErrorAction SilentlyContinue) { "
                "  & robocopy $childActive $stageTmp /E /COPY:DATS /DCOPY:DAT /R:1 /W:1 /NFL /NDL /NJH /NJS /NP | Out-Null; "
                "  if ($LASTEXITCODE -le 7) { $copiedStage = $true } "
                "} "
                "if (-not $copiedStage) { "
                "  if (Test-Path -LiteralPath $childActive) { "
                "    Get-ChildItem -LiteralPath $childActive -Force -ErrorAction SilentlyContinue | "
                ...
                "} "
                "if ($childTempDrive) { "
                "  try { zfs unmount $child | Out-Null } catch {}; "
                "  if ($childDriveOrig -and $childDriveOrig -ne '-' -and $childDriveOrig -notmatch '^(off|none)$') { "
                "    try { zfs set (\"driveletter=\" + $childDriveOrig) $child | Out-Null } catch {} "
                "  } else { "
                "    try { zfs inherit driveletter $child | Out-Null } catch {} "
                "  } "
                "}; "
                "if ($datasetTempDrive) { "

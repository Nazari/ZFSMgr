    ...
    def _run_assemble_psrp_command(
        ...
        def _build_child_script(dataset_name: str, mountpoint_hint: str, child_name: str) -> str:
            ...
            return (
                ...
                "  foreach ($line in $mountedRows) { "
                "    $parts = ($line -split '\\s+'); "
                "    if ($parts.Length -ge 2 -and $parts[0] -eq $ds) { $p = $parts[1]; break } "
                "  } "
                "} catch { $p='' }; "
                "if ($p -and (Test-Path -LiteralPath $p)) { return $p }; "
                "try { zfs mount $ds | Out-Null } catch {}; "
                "try { "
                "  $mountedRows = zfs mount; "
                "  foreach ($line in $mountedRows) { "
                "    $parts = ($line -split '\\s+'); "
                ...
                "} catch { $p='' }; "
                "if ($p -and (Test-Path -LiteralPath $p)) { return $p }; "
                "try { $dl = (zfs get -H -o value driveletter $ds).Trim() } catch { $dl='' }; "
                "if ($dl -and $dl -ne '-' -and $dl -notmatch '^(off|none)$') { "
                "  $cand = ($dl.TrimEnd(':') + ':\\'); "
                "  if (Test-Path -LiteralPath $cand) { return $cand } "
                "} "
                "try { $mp = (zfs get -H -o value mountpoint $ds).Trim() } catch { $mp='' }; "
                "if ($mp -and $mp -ne 'none' -and (Test-Path -LiteralPath $mp)) { return $mp }; "
                "if ($hint -and (Test-Path -LiteralPath $hint)) { return $hint }; "
                "return ''; "
                "}; "
                "$mp = & $resolvePath $dataset $mpHint; "
                "if (-not (Test-Path -LiteralPath $mp)) { "

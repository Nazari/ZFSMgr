...
class App(tk.Tk):
    ...
    def _assemble_dataset_plan(self) -> None:
        ...
        assemble_cmd = (
            ...
            "CHILD_TEMP=0; "
            "CHILD_PATH=''; "
            "if [ -n \"$child_active\" ] && [ \"$child_active\" = \"$dest\" ]; then "
            "child_active=''; "
            "fi; "
            "if [ -n \"$child_mp\" ] && [ \"$child_mp\" = \"$dest\" ]; then "
            "child_mp=''; "
            "fi; "
            ...
            "if [ -n \"$child_active_retry\" ] && [ -d \"$child_active_retry\" ]; then "
            "CHILD_PATH=\"$child_active_retry\"; "
            "else "
            "echo \"[ASSEMBLE][ERROR] cannot mount child dataset: $child\"; "
            "exit 34; "
            "fi; "
            "else "

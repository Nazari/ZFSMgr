...
可以正常使用的部分：读取和修改数据集与存储池、快照、克隆、ZFS 权限、*组装*、
**机器之间的快照`复制`与`同步快照`**、**后台作业**，以及代理的日志和心跳。

**`拆分` 在 Windows 上不可用**，`到目录` 也不可用。原因见下文。

该列表并非写死在应用中：**由代理在被询问状态时自行声明**，因此只要安装了覆盖更多功能的
版本，列表就会自动更新。

## 为什么 Windows 上没有 `拆分`（而 `组装` 有）

这里以前写着两者都可用，"只是结果与 Unix 上看起来不同"。在真实机器上验证后发现情况更糟，
因此 **`拆分` 在 Windows 上已经取消**。

在那里，数据集**不会嵌套在父级之下**：每个都以链接的形式挂载在驱动器根目录，名称取自其
最后一段。`winpool/父/子` 位于 `Z:\子`，而不是 `Z:\父` 之内，后者看起来是空的。

实际影响：拆分 `Z:\数据\照片` 后，内容会出现在 `Z:\照片`，即驱动器根目录。ZFS 的层级是
正确的，但**您的文件会从原来的位置消失，出现在磁盘的顶层**，而操作却报告成功。

这无法修复：Windows 上 `zfs set mountpoint` 只接受驱动器号、`none` 或 `legacy`，因此无法
让子数据集挂载到父级之内。代理已不再在那里提供该功能，界面会显示为禁用并给出原因。

**`组装` 可以正常工作**，并已验证：数据集会原地变回目录，位于父级之内。

...
**一处补充说明（发布之后实测得出）：**「不会嵌套」描述的是所见的现象，其成因是**挂载
顺序**，而非某种禁止。若父级尚未挂载而先挂载了子级，子级确实会出现在父级之内；一旦父级
挂载，这部分内容就会被父级的挂载**遮蔽**而不再可见。也就是说，并不是 ZFS 禁止嵌套：在
一切都已挂载的正常情形下，驱动器根目录上的扁平挂载才是最终可见的结果。

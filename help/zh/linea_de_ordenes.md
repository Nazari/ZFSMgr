...
`ro` 就是 `rollback`。有四个可以只写一个字母：`b`、`g`、`q` 和 `y`。

**不需要记住要写几个字母。** 输入您认为合适的长度；如果不够，命令行会列出它在哪几个之间
犹豫：
...
> cl
「cl」不明确：clear, clone, cls
```

这与不存在的命令不同，后者仍会提示「未知命令」。这份列表与 TAB 补全提供的完全一致，因此
补全和缩写不会出现分歧。

**完全匹配始终优先。** `job` 是一个命令，`jobs` 是另一个：输入 `job` 执行的就是 `job`，
即使 `jobs` 也以此开头。没有这条规则，就没有办法输入较短的那个。`hold` 与 `holds`、
`schedule` 与 `schedules`、`export` 与 `export-trust` 同理。

缩写**只适用于命令本身**，也就是第一个词。后面的内容——位置、选项、取值——都要完整输入。
